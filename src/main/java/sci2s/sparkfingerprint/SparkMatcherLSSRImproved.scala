...
import sci2s.mrfingerprint.PartialScoreLSSRImproved
import sci2s.mrfingerprint.PartialScore
import sci2s.mrfingerprint.Minutia


object SparkMatcherLSSRImproved {
  
  val usage = """
    Usage: SparkMatcherLSSRImproved
      [--template-file path]
      [--map-file path]
      [--output-dir path]
      [--num-partitions num]
    ...
    val scores = templateLS.groupByKey().flatMapValues({ tlsarray =>
        ...
        inputLS.value.map { case (ifpid, (ils, minutiae)) =>
          ...
          val score = tlsarray.map ({ ls =>
            new PartialScoreLSSRImproved(ls, ils.asInstanceOf[Array[LocalStructure]])
            }).filter(! _.isEmpty).reduce(_.aggregateSinglePS(_).asInstanceOf[PartialScoreLSSRImproved]).computeScore(minutiae)
